...
class CanSatMissionControl:
    def __init__(self, root):
        ...
        # Aquí van las entradas de PRESIÓN ATMOSFÉRICA (kPa)
        self.pressure_value = tk.DoubleVar(value=101.3)
        
        # Aquí va la entrada de TEMPERATURA AMBIENTAL (°C)
        self.temperature_value = tk.DoubleVar(value=24.8)
        
        # Aquí va la entrada de ALTITUD (metros sobre el nivel del mar)
        self.altitude_value = tk.DoubleVar(value=1245.8)
        
        # Aquí va la entrada de TASA DE CAMBIO DE ALTITUD (m/s)
        self.altitude_rate = tk.DoubleVar(value=12.4)
        
        # Aquí va la entrada de LATENCIA DE SEÑAL (milisegundos)
        self.signal_latency = tk.DoubleVar(value=24)
        
        # Datos para gráficas
        # Aquí van los datos de DISTRIBUCIÓN DE VELOCIDAD en el tiempo
        self.velocity_time = np.linspace(-120, 0, 100)  # Últimos 120 segundos
        self.velocity_data = 10 * np.sin(self.velocity_time/20) * np.exp(self.velocity_time/60) + 15
        
        # Aquí van los datos de ACELERACIÓN G-FORCE en el tiempo
        self.gforce_time = np.linspace(-1.5, 4.0, 100)
        self.gforce_data = 2 + 3*np.exp(-(self.gforce_time-0.5)**2/0.3) + 1.5*np.exp(-(self.gforce_time-2)**2/0.5)
    ...
    def create_main_content(self):
        ...
        self.create_circular_gauge(top_row, "ATMOSPHERIC", "101.3", "kPa", 
                                   "Pressure", self.blue_accent, 0.7, 'pressure').pack(side=tk.LEFT, padx=(0, 15))
        
        # Tarjeta de temperatura
        self.create_circular_gauge(top_row, "ENVIRONMENT", "24.8", "°C", 
                                   "Temperature", self.orange_accent, 0.4, 'temperature').pack(side=tk.LEFT, padx=(0, 15))
    ...
    def create_circular_gauge(self, parent, label_top, value, unit, label_bottom, color, fill_percent, gauge_type):
        ...
        canvas.create_arc(10, 10, 110, 110, start=0, extent=359.9, 
                         outline='#3a4f6f', width=12, style=tk.ARC, tags='bg_arc')
        ...
        canvas.create_arc(10, 10, 110, 110, start=90, extent=-extent, 
                         outline=color, width=12, style=tk.ARC, tags='progress_arc')
        
        # Valor central
        value_label = tk.Label(canvas, text=value, font=('Arial', 28, 'bold'), 
                              bg=self.bg_panel, fg=self.text_white)
        value_label.place(relx=0.5, rely=0.4, anchor='center')
        
        unit_label = tk.Label(canvas, text=unit, font=('Arial', 11), 
                             bg=self.bg_panel, fg=self.text_gray)
        unit_label.place(relx=0.5, rely=0.62, anchor='center')
    ...
    def create_altitude_card(self, parent):
        ...
        self.altitude_label = tk.Label(alt_value_frame, text="1,245.8", 
                            font=('Arial', 36, 'bold'), 
                            bg=self.bg_panel, fg=self.text_white)
        ...
        self.altitude_rate_label = tk.Label(rate_frame, text="+12.4 m/s", 
        ...
        self.latency_label = tk.Label(lat_value_frame, text="24", 
                            font=('Arial', 36, 'bold'), 
                            bg=self.bg_panel, fg=self.text_white)
    ...
    def update_sensor_data(self):
        """Actualizar valores de sensores en tiempo real"""
        if self.mission_running:
            # Simular variación en los sensores
            # En producción, aquí se recibirían DATOS REALES del CanSat
            
            # PRESIÓN ATMOSFÉRICA (kPa) - Simular cambios pequeños
            new_pressure = 101.3 + np.sin(self.elapsed_seconds/10) * 2 + np.random.normal(0, 0.1)
            ...
            if hasattr(self, 'pressure_canvas'):
                fill = (new_pressure - 95) / 15  # Normalizar entre 95-110 kPa
            ...
            # TEMPERATURA (°C) - Simular cambios
            new_temp = 24.8 + np.sin(self.elapsed_seconds/15 + 1) * 3 + np.random.normal(0, 0.2)
            self.temperature_value.set(new_temp)
            if self.temp_label:
                self.temp_label.config(text=f"{new_temp:.1f}")
            # Actualizar arco de progreso
            if hasattr(self, 'temp_canvas'):
                fill = (new_temp - 15) / 25  # Normalizar entre 15-40°C
                fill = max(0, min(1, fill))
            ...
            # ALTITUD (metros) - Simular ascenso/descenso
            new_altitude = 1245.8 + self.elapsed_seconds * 2.5 + np.sin(self.elapsed_seconds/8) * 50
            self.altitude_value.set(new_altitude)
            if self.altitude_label:
                self.altitude_label.config(text=f"{new_altitude:,.1f}")
            
            # TASA DE CAMBIO DE ALTITUD (m/s)
            new_rate = 12.4 + np.sin(self.elapsed_seconds/7) * 8
            ...
            # LATENCIA DE SEÑAL (ms)
            new_latency = 24 + np.random.normal(0, 3)
            new_latency = max(10, min(100, new_latency))
            self.signal_latency.set(new_latency)
            if self.latency_label:
                self.latency_label.config(text=f"{int(new_latency)}")
            
            # Agregar mensaje al log ocasionalmente
    ...
    def update_graphs(self):
        """Actualizar gráficas en tiempo real con datos simulados"""
        if self.mission_running:
            # ACTUALIZAR GRÁFICA DE VELOCIDAD
            # Rotar datos (mover ventana de tiempo)
            self.velocity_time = np.roll(self.velocity_time, -1)
            self.velocity_time[-1] = 0  # Nuevo punto en tiempo actual
            
            # Generar nuevo dato de velocidad
            # En producción, aquí vendrían DATOS REALES de velocidad
            ...
            # Actualizar área rellena
            self.velocity_ax.collections.clear()
            self.velocity_ax.fill_between(self.velocity_time, self.velocity_data, 
                                          alpha=0.1, color=self.blue_accent)
            
            self.velocity_canvas.draw()
            
            # ACTUALIZAR GRÁFICA DE G-FORCE
            # Rotar datos
            self.gforce_time = np.roll(self.gforce_time, -1)
            self.gforce_time[-1] = self.gforce_time[-2] + 0.055  # Incrementar tiempo
            
            # Generar nuevo dato de aceleración
            # En producción, aquí vendrían DATOS REALES de aceleración
            ...
            # Actualizar área rellena
            self.gforce_ax.collections.clear()
            self.gforce_ax.fill_between(self.gforce_time, self.gforce_data, 
                                        alpha=0.15, color=self.orange_accent)
            
            # Ajustar límites del eje X para seguir el tiempo
            ...
            # Agregar mensaje de telemetría al log ocasionalmente
            if self.elapsed_seconds % 3 == 0:
                self.add_log_message("DATA", 
                    f"VEL:{new_velocity:.1f}m/s G:{new_gforce:.2f}")
